...
现有 Git Bootstrap 已接受一个 `Minor` 风险：`git worktree add` 失败时，可能残留目录、分支或 Worktree metadata。它不是对象存储或外部服务问题。该风险由 Codex 先读取实际目录、branch、`git worktree list --porcelain`、提交和工作区状态，再调用 Worktree Doctor 判断是安全修复还是转为 `BLOCKED`；不得在未知状态下自动强删。
...
- `git worktree add` 失败时不得自动强删；保留错误、目录、分支和 metadata 现场，先由 Codex/Doctor 检查实际状态。
...
| 已接受的 Minor 风险 | Codex 检查 + Doctor 受控修复 | 避免 Worktree 创建失败后误删有效目录、分支或 metadata |
...
本设计已获 Human 确认。下一步只为 MVP 0 编写逐步实施计划；MVP 1、2、3 保持路线图状态，不并行开工。MVP 0 完成并验收后，才进入 MVP 1。
