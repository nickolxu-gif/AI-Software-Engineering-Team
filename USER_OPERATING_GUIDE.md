...
总览页的“提交新工程需求”只需要填写：标题、要达成的目标，以及可选背景。提交后它显示为“待处理需求”，并只进入本机 SQLite 收件箱；页面不会据此创建任务、分支、Worktree、Git 提交、合并、push、发布或审批。收件箱最多保留 100 条不可变审计记录；容量已满时会拒绝新请求，不会自行删除历史记录。
